...
#include "audio/audiointerfaceforqt.h"
...
void PageAudioInputOutput::onDeviceSelectionChanged(int row) {
    ...
    // TODO
    // for (int rate : info.supportedSampleFormats()) {
    //     if (rate < 9000) {
...
void PageAudioInputOutput::addDevice(QAudioDevice info) {
    if (mDeviceSelection->findText(info.deviceName()) == -1) {
        mDeviceSelection->addItem(info.deviceName(), QVariant::fromValue<QAudioDevice>(info));
    }
}
...
void DeviceLoaderThread::run() {
    QList<QAudioDevice> deviceInfos(QAudioDeviceInfo::audioInputs());
    // deviceInfos.append()
    int progress = 0;
