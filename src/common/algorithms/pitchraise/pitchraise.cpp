...
PitchRaise::PitchRaise(const Piano &piano, const AlgorithmFactoryDescription &description) : Algorithm(piano, description),
...
void PitchRaise::algorithmWorkerFunction()
{
    ...
    for (int i = 0; i < mNumberOfKeys; ++i)
    {
        ...
        if (key.getMeasuredInharmonicity() > 1E-10)
        {
            ...
            if (s == 0)
            {
                LogI("Found recorded key number %d in the  left section with B=%lf.", i, B);
            }
            else
            {
                LogI("Found recorded key number %d in the right section with B=%lf.", i, B);
            }
            double x = i, y = -log(B);
            N[s]++;
            X[s] += x;
    ...
    if (N[0] < 2)
    {
        ...
        MessageHandler::send<MessageCaluclationProgress>(MessageCaluclationProgress::CALCULATION_FAILED,
    ...
    if (mKeyNumberOfA4 <= 13 or mNumberOfKeys - mKeyNumberOfA4 <= 13)
        return;
